...
const fetchPosts = async () => {
	try {
		const response = await axios.get("http://localhost:5181/api/post/all");
...
const submitPost = async () => {
	...
	try {
		await axios.post("http://localhost:5181/api/Post", formData, {
	...
	<div class="container">
		...
		<div v-if="posts.length" class="posts-container">
			<div v-for="post in posts" :key="post.id" class="post">
				...
				<a v-if="post.imageUrl" :href="'http://localhost:5181' + post.imageUrl" target="_blank">
					<img :src="'http://localhost:5181' + post.imageUrl" alt="Uploaded image" />
				</a>
			</div>
		</div>
